...
The setup here is the same as in previous analyses 
...
I am going to explore some of the distances between the prototype and the other scenarios. To begin let me define a function to calculate euclidean distance between two points in 3 dimensional space. 
...
Now let's see if I can make function run on my actual values
...
Alright, now we have the distances on the three axes for the scenarios. Can I join them with the big dataset.
...
Got the distances, now let's do some summary plots. 
...
We see an interesting pattern here. In the plot that includes all scenarios, there seems to be no relationship between average immorality and distance from the prototype. When we exclude purity, however, the relationship becomes negative, as we would have expected. 

Nonetheless, there is an issue here: including the object's potency in our calcculation of distance seems a bit dubious. This is because our previous analysis shows that as the object's potency increases, immorality decreases. Here, then, we are wrongly "penalizing" the scenarios that have more vulnerable patients that that included in our prototype - i.e. person -, which is fairly neutral. For that reason, I am going to calculate distance from the prototype, only considering behavior evaluation and behavior potency. 
